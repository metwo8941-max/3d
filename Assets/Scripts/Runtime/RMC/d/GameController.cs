...
namespace RMC.d.Scenes
{
    ...
    public class GameController : MonoBehaviour
    {
        ...
        private bool _wasMoveInputActive = false;
        ...
        private void CheckUserInput()
        {
            ...
            Vector2 touchMoveInputVector2 = GameView != null ? GameView.TouchMoveInput : Vector2.zero;

            if (touchMoveInputVector2.sqrMagnitude > moveInputVector2.sqrMagnitude)
            {
                moveInputVector2 = touchMoveInputVector2;
            }

            bool didUseTouchMoveInput =
                touchMoveInputVector2.magnitude > GameModel.MoveInputMinimumMagnitude &&
                touchMoveInputVector2.sqrMagnitude >= moveInputVector2.sqrMagnitude;
            bool isMoveInputActive = moveInputVector2.magnitude > GameModel.MoveInputMinimumMagnitude;
            ...
            if (isMoveInputActive)
            {
                ...
                if (_movePlayerInputAction.WasPerformedThisFrame() || (didUseTouchMoveInput && !_wasMoveInputActive))
                {
                    PlayAudioClip(PlayerMoveAudioClip);
                }
            }

            _wasMoveInputActive = isMoveInputActive;

            // Only allow jump when grounded
            bool shouldJump = _jumpPlayerInputAction.WasPerformedThisFrame();
            if (GameView != null && GameView.ConsumeTouchJumpRequest())
            {
                shouldJump = true;
            }

            if (shouldJump && IsPlayerGrounded)
            {
        ...
        private void dSingleton_OnIsDarkChanged(bool value)
        {
            ...
            _gameModel.Instructions.Value = Application.isMobilePlatform
                ? "Touch: Move, Jump"
                : "Instructions: WASD/Arrows, Spacebar, R, T";
        }
